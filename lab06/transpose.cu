...
__global__ void transpose(float *odata, const float *idata) {
    ...
    __shared__ float cacheData[TILE_DIM * TILE_DIM];

    for (int j = 0; j < TILE_DIM; j+= BLOCK_ROWS) {
        cacheData[(threadIdx.y + j) * TILE_DIM + threadIdx.x] = idata[(y + j) * width + x];
    }
    ...
    for (int j = 0; j < TILE_DIM; j+= BLOCK_ROWS) {
        odata[(y + j) * width + x] = cacheData[threadIdx.x * TILE_DIM + threadIdx.y + j];
    }
}
