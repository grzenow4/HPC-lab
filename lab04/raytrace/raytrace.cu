...
// __constant__ Sphere constSpheres[SPHERES];
...
__global__ void kernel(Sphere *spheres, unsigned char* bitmap) {
    ...
    __shared__ Sphere cacheSpheres[SPHERES];
    for (int i = threadIdx.x + threadIdx.y * blockDim.x; i < SPHERES; i += blockDim.x * blockDim.y)
        cacheSpheres[i] = spheres[i];

    for (int i = 0; i < SPHERES; i++) { 
        float colorFalloff;
        float depth = cacheSpheres[i].hit(bitmapX, bitmapY, &colorFalloff);
        
        if (depth > maxDepth) { 
            red = cacheSpheres[i].red * colorFalloff;
            green = cacheSpheres[i].green * colorFalloff;
            blue = cacheSpheres[i].blue * colorFalloff;
            maxDepth = depth; 
        }
    }
    ...
    // for (int i = 0; i < SPHERES; i++) { 
    //     float colorFalloff;
    //     float depth = constSpheres[i].hit(bitmapX, bitmapY, &colorFalloff);
        
    //     if (depth > maxDepth) { 
    //         red = constSpheres[i].red * colorFalloff;
    //         green = constSpheres[i].green * colorFalloff;
    //         blue = constSpheres[i].blue * colorFalloff;
    //         maxDepth = depth; 
    //     }
    // }
...
int main(void) {
    ...
    // HANDLE_ERROR(cudaMemcpyToSymbol(constSpheres, hostSpheres, sizeof(Sphere) * SPHERES));
    free(hostSpheres);
